...
- **[Download OrcaSlicer](https://github.com/OrcaSlicer/OrcaSlicer/releases)** — use the stable course-approved release from the official GitHub project. The successful July 2026 course files were generated by OrcaSlicer 2.4.2.
...
The successful July G-code files begin with `generated by OrcaSlicer 2.4.2` and identify `printer_model = Creality K2 Pro` and `printer_settings_id = Creality K2 Pro 0.4 nozzle`. Some also contain a `Creality_Print` comment or `printer_agent = crealityprint`; those strings came from the imported Creality machine profile/start sequence. They do **not** change the required application: students use **OrcaSlicer**.
...
## Downloadable examples from the July workshop

- **[Successful OrcaSlicer/K2 Pro G-code example](docs/printing/examples/successful-july-2026-k2pro-orcaslicer.gcode)** — the corrected file attached to the July 21, 2026 email. Its SHA-256 is `7c6b491ecef6f1c900149f7e8c0183ea1a13d74038cc1b1dcffcf69e2f7447c3`.
...
The July G-code is an **archival reference, not a print-ready course file**. It was sliced for a particular geometry, revision, profile, material assumption, and machine state. Students may inspect its header to see the OrcaSlicer version and K2 Pro profile identifier, but must not run, rename, copy, or edit it for the current project. Each team must generate new G-code from its own checked geometry and the current approved profile.
...
| **44.57 g** in the prior example | **52.30 g** in the prior example |
